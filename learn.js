document.addEventListener('DOMContentLoaded', function() {
    // --- PHẦN CẤU HÌNH ---
    // !!! QUAN TRỌNG: Hãy giữ API Key này cẩn thận và không chia sẻ công khai.
    // Đây là cách dùng cho dự án cá nhân, không an toàn cho sản phẩm thực tế.
    ...
    const startButton = document.querySelector('.topic-card:not(.disabled) .topic-button');
...
// --- HÀM GỌI GEMINI API (PHIÊN BẢN NÂNG CẤP CHỐNG LẶP TỪ) ---
    async function getNewWordFromAI() {
        ...
        const exclusionListString = learnedWords.length > 0 ? `The word MUST NOT be in the following list: [${learnedWords.join(', ')}]` : '';
        ...
        try {
            ...
            updateWordCard(currentWordData, imageUrl); // Truyền thêm imageUrl            

        } catch (error) {
            console.error('Error fetching from Gemini API:', error);
            alert('Đã có lỗi xảy ra khi lấy từ mới. Vui lòng thử lại. Chi tiết lỗi ở trong Console.');
            showTopicSelection();
        } finally {
...
// --- HÀM MỚI: LẤY HÌNH ẢNH TỪ PIXABAY ---
    async function getImageForWord(word) {
        const pixabayUrl = `https://pixabay.com/api/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(word)}&image_type=photo&orientation=horizontal&per_page=3`;
...
// --- HÀM CẬP NHẬT GIAO DIỆN (PHIÊN BẢN CÓ HÌNH ẢNH) ---
    function updateWordCard(data, imageUrl) {
        ...
        document.getElementById('example-general').innerHTML = data.example_general.replace(new RegExp(`\\b(${data.word})\\b`, 'gi'), '<strong>$1</strong>');
        document.getElementById('example-domain').innerHTML = data.example_domain.replace(new RegExp(`\\b(${data.word})\\b`, 'gi'), '<strong>$1</strong>');
